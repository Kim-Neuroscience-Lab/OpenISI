...
mod garrett {
    ...
    mod golden {
        ...
        fn split_fuse_match_genuine_snlc_matlab_live() {
            ...
            const N: usize = 64;
            let pixpermm = 20.0;
            // Smooth retinotopy gradients (deg) + eccentricity; a central disk patch
            // that reaches past the R=30° eccentricity limit so the split pass acts.
            let lin = |i: usize, a: f64, b: f64| a + (b - a) * i as f64 / (N - 1) as f64;
            let azi = Array2::from_shape_fn((N, N), |(_, c)| lin(c, -40.0, 40.0));
            let alt = Array2::from_shape_fn((N, N), |(r, _)| lin(r, -40.0, 40.0));
            let ecc = Array2::from_shape_fn((N, N), |(r, c)| {
                (azi[[r, c]].powi(2) + alt[[r, c]].powi(2)).sqrt()
            });
            let im = Array2::from_shape_fn((N, N), |(r, c)| {
                let (dr, dc) = (r as f64 - 31.5, c as f64 - 31.5);
                (dr * dr + dc * dc).sqrt() < 26.0
            });
            let to_u8 = |a: &Array2<f64>| -> Vec<u8> { a.iter().map(|&v| u8::from(v != 0.0)).collect() };
            ...
            .remove(0);
            let ours_split = super::split_patches_x(&im, &azi, &alt, &ecc, pixpermm);
            let d_split = count_differing(&ours_split, &to_u8(&g_split));
            eprintln!("splitPatchesX vs GENUINE MATLAB (live): differing px = {d_split}");
            assert_eq!(d_split, 0, "split_patches_x diverges from genuine splitPatchesX.m");

            // fuse (on the matched split output)
            let g_fuse = oracle::snlc(
                "fusepatchesx",
                &[g_split.clone(), azi.clone(), alt.clone()],
                &[("pixpermm", pixpermm)],
            )
            .remove(0);
            let ours_fuse = super::fuse_patches_x(&ours_split, &azi, &alt, pixpermm);
            let d_fuse = count_differing(&ours_fuse, &to_u8(&g_fuse));
            eprintln!("fusePatchesX vs GENUINE MATLAB (live): differing px = {d_fuse}");
            assert_eq!(d_fuse, 0, "fuse_patches_x diverges from genuine fusePatchesX.m");
        }
    }
}
