...
fn analyzed_oisi_conforms_to_schema() {
    // A real analyzed `.oisi` must conform to `oisi_schema::SCHEMA`. Uses the
    // committed SYNTHETIC analyzed baseline (tiny, deterministic) so this runs
    // everywhere, including CI. (Real R43 data is gitignored / never published; the
    // synthetic baseline exercises the same schema leaves a real analysis produces.)
    let oisi = manifest_dir().join("tests/fixtures/synthetic/smoke.baseline.oisi");
    assert!(
        oisi.exists(),
        "committed synthetic baseline missing — regenerate with `gen_synthetic_smoke` + \
         `capture_baseline`: {}",
        oisi.display()
    );
