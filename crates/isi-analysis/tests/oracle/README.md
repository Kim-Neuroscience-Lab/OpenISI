...
- **Octave's `image` package lacks `roifilt2`** (`exist('roifilt2')==0` in 11.2.0),
  ...
  be executed natively** in the Octave env. **STATED PLAINLY (a real, not-yet-closed
  gap):** the *existing committed* `smpatch_*`/`splitx_*`/`fusex_*` golden fixtures were
  generated by the genuine `.m` running against a **`roifilt2` SHIM** (a self-authored
  stand-in) — exactly the kind of shim-contaminated oracle this goal exists to
  eliminate. So those goldens are **NOT** clean genuine-reference values and must not be
  read as such. They are pending honest resolution: either downgraded to explicitly-
  labelled regression-locks with this gap front-and-centre, or dropped. We will **not**
  author a `roifilt2` replacement to "fix" them — that self-written logic in the oracle
  path is the circularity the goal forbids. (Gprocesskret's **no-smoothing** branch —
