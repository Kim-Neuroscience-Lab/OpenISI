...
fn load_for_target(
    ...
    let contents = std::fs::read_to_string(path).map_err(|e| {
        ParamsError::Config(format!("Failed to read {label} ({}): {e}", path.display()))
    })?;
    ...
    //    completes.
    let mut known_paths: std::collections::HashSet<&'static str> = std::collections::HashSet::new();
    ...
    registry.batch(|reg| -> ParamsResult<()> {
        for def in PARAM_DEFS.iter() {
            ...
            if let Some(val) = navigate_toml(&table, def.toml_path) {
                ...
                // context.
                let pv = toml_to_param_value(def.id, val)
                    .map_err(|e| ParamsError::Config(format!("{label}: {e}")))?;
                match layer {
...
pub fn load_shipped_analysis(registry: &mut Registry, path: &Path) -> ParamsResult<()> {
    load_for_target(
        registry,
        path,
        PersistTarget::Analysis,
        "analysis.toml (shipped)",
        LoadLayer::Shipped,
    )
}
...
pub fn load_shipped_experiment(registry: &mut Registry, path: &Path) -> ParamsResult<()> {
    load_for_target(
        registry,
        path,
        PersistTarget::Experiment,
        "experiment.toml (shipped)",
        LoadLayer::Shipped,
    )
}
...
pub fn load_experiment_file(registry: &mut Registry, path: &Path) -> ParamsResult<ExperimentMeta> {
    ...
    registry.batch(|reg| -> ParamsResult<()> {
        for def in PARAM_DEFS.iter() {
            ...
            if let Some(val) = navigate_toml(&table, def.toml_path) {
                ...
                let pv = toml_to_param_value(def.id, val)
                    .map_err(|e| ParamsError::Config(format!("{}: {e}", path.display())))?;
                reg.set(def.id, pv)?;
            }
        }
        Ok(())
...
fn write_toml(
    ...
    std::fs::write(path, toml_str).map_err(|e| {
        ParamsError::Config(format!("Failed to write {} ({label}): {e}", path.display()))
    })
}
...
fn save_user_for_target(
    ...
    for def in PARAM_DEFS.iter() {
        ...
        if !registry.is_user_override(def.id) {
            continue;
        }
...
fn insert_at_path(
    ...
    for part in head {
        ...
        current = entry.as_table_mut().ok_or_else(|| {
            ParamsError::Config(format!(
                "TOML path conflict at '{part}' in '{path}': expected table"
            ))
        })?;
    }
...
fn toml_to_param_value(id: ParamId, val: &toml::Value) -> ParamsResult<ParamValue> {
    ...
    let json = serde_json::to_value(val).map_err(|e| {
        ParamsError::Config(format!(
            "converting TOML value at {} to JSON: {e}",
            def.toml_path
        ))
    })?;
...
/// then JSON → TOML through serde.
///
/// In principle this is infallible for real param values (the JSON form
/// is a scalar/array/string with no nulls or NaNs), but the JSON → TOML
/// conversion is fallible at the serde level (TOML can't represent
/// `null` or non-finite floats). Returning a typed error instead of
/// panicking respects the no-silent-fallback / no-`.expect` discipline
/// even when the error path is theoretically unreachable. A NaN that
/// somehow reached this function should surface as a config error, not
/// crash the writer thread.
fn param_value_to_toml(value: &ParamValue) -> crate::error::ParamsResult<toml::Value> {
    let json = crate::param_json::to_json(value);
    toml::Value::try_from(json).map_err(|e| {
        crate::error::ParamsError::Config(format!(
            "ParamValue {:?} could not be represented as TOML: {e}",
            value
        ))
    })
}
