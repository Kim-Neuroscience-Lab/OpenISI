...
The Registry's `PersistTarget::Rig` params; sections appear automatically
based on each param's `toml_path`. The shape is generated by
`save_rig` from `PARAM_DEFS` — adding a new Rig param adds a row here
without any hand-edit. Current shape (abbreviated; see
`config/rig.toml` for the full file):

```toml
[camera]
exposure_us = 1000
binning = 4
um_per_pixel = 20.0  # rig-hardware fact; absorbed into AcquisitionProperties at analyze time
...
drop_detection_threshold = 1.5  # post-hoc Δus ratio (dataset.rs), not live
...
**Note**: there is no `[analysis]` section in `rig.toml` anymore.
Analysis tunables live in:
- `config/analysis.toml` — bootstrap method choices per stage
- `methods/<stage>.rs` — `const FOO_DEFAULT` per tunable
- `.oisi /analysis_params` — per-file overrides (set via the UI's
  "Method Tunables" cards which call `set_method_tunable`)

See `docs/oisi.schema.json` for the full `.oisi` layout.
...
### Configuration model (Registry + macro-driven param table)

> **As-built note (2026-05-23).** Earlier drafts of this document
> proposed one typed struct per domain (`RigConfig`, `Experiment`,
> etc.). The codebase instead uses a *flat parameter registry* driven
> by the `define_params!` macro. Both designs satisfy the
> domain-separation principles (each param has a clear
> `PersistTarget`); the registry approach won on uniformity (one
> validation / serialization / UI-descriptor path for every param).
> The typed-struct alternative is captured in **Future direction**
> below for anyone considering a refactor.

The `Registry` (`src-tauri/src/params/registry.rs`) holds every
configurable parameter — across Rig, Experiment, UI-state, and System
tuning — in a flat `Vec<ParamValue>` indexed by a generated `ParamId`
enum. The macro `define_params!` (in `src-tauri/src/params/macros.rs`)
takes the canonical declaration in `src-tauri/src/params/definitions.rs`
and generates:

- the `ParamId` enum (one variant per param)
- the static `PARAM_DEFS` table (name / type / TOML path / persist-target / group / constraints)
- typed getters/setters on `Registry`: `reg.camera_exposure_us()`, `reg.set_camera_exposure_us(1500)?`
- the TOML I/O bridge (rig.toml ↔ params with `PersistTarget::Rig`, etc.)
- the UI-descriptor bridge consumed by the frontend's `buildParamGroup`

Adding a new parameter is a single edit to `definitions.rs`:

```rust
CameraExposureUs: U32 = 1000,
    "camera.exposure_us", Rig, Camera,
    "Exposure", "µs", StaticConstraint::RangeU32(1, 1_000_000);
```

The macro handles everything downstream — TOML I/O, validation, UI
descriptor, snapshot for thread-spawn, change events. There are no
hand-maintained per-domain structs.

### Per-stage analysis tunables (method enums + descriptors)

Analysis-side per-stage parameters live OUTSIDE the registry, in
tagged-enum method choices (`crates/isi-analysis/src/methods/*`).
Each stage's enum variant carries `Option<T>` fields with a colocated
`const FOO_DEFAULT`. The active method choice + any explicit overrides
are persisted into `.oisi /analysis_params` per file, so each .oisi
records the exact methods that produced its results.

```rust
pub enum SignMapSmoothingMethod {
    Gaussian {
        #[serde(default, skip_serializing_if = "Option::is_none")]
        sigma_um: Option<f64>,  // None ≡ const GAUSSIAN_SIGMA_UM_DEFAULT
    },
}
```

These tunables are first-class in the UI via the
`MethodTunables` trait (`methods::descriptor`). Every method enum
implements `tunables()` (returns descriptors for the current variant's
fields) and `set_tunable(name, value)` (mutates one field). The Tauri
commands `get_method_tunables` / `set_method_tunable` expose the
trait; the analysis view renders one editable card per stage with one
input per tunable. SSoT:
- defaults: code-side `const FOO_DEFAULT`
- per-file overrides: `.oisi /analysis_params` JSON
- UI view: `get_method_tunables` reads from the active .oisi

### Acquisition properties (capture-time facts)

Stimulus geometry and camera calibration are NOT algorithm choices;
they describe the capture. They live in two places:
- In the Registry as `PersistTarget::Experiment` (stimulus geometry)
  + `PersistTarget::Rig` (camera µm/pixel).
- Snapshotted into the `.oisi` as `/rig_params` + `/experiment_params`
  JSON at acquisition time.

At analyze time, `AcquisitionProperties::from_oisi_attrs` reads them
back, stamping the result with a `ProvenanceLevel` (`Full` / `Partial { missing }` / `Defaulted`).
Every consumer MUST match on `provenance` — the type forbids silent
fallbacks, satisfying the project's "no fallbacks / provenance always"
principle.

### Future direction: typed-struct-per-domain refactor

A typed-struct-per-domain design — `pub struct RigConfig { camera:
CameraDefaults, geometry: RigGeometry, ... }` etc. — is a viable
alternative to the current registry-driven approach. It would trade
the macro's uniformity (one path for TOML / UI / validation / events)
for stronger compile-time type checking and IDE discoverability.
Indicative scope: weeks of refactor work; touches every consumer of
`Registry::get` / typed-getters; needs a new bridge layer to keep
the macro-generated UI descriptors working (or eliminates them by
generating descriptors from the typed structs via a derive macro).
Not recommended without a concrete pain point the registry can't
address.
...
These remain plain enums because they appear in multiple contexts
(serde, shaders, UI dropdowns) and don't fit the registry's
`ParamValue` discriminants.
